...
set "OPENOCD_ROOT=D:\MyTools\OpenOCD_ArmGCC\OpenOCD&ArmGCC\DevEnv\openocd-v0.12.0-i686-w64-mingw32"
REM    示例："OPENOCD_ROOT=D:\Tools\openocd-v0.12.0-i686-w64-mingw32"

REM 2. DAPLink 配置文件（绝对路径，到 .cfg 文件）
set "DAPLINK_CFG=D:\MyTools\OpenOCD_ArmGCC\ozone_daplink.cfg"
...
if not exist "%TARGET_CFG%" (
    echo [错误] 未找到目标芯片配置：%TARGET_CFG%
    echo 你用的是 STM32G4，请确认路径正确！
    pause
    exit /b 1
)
...
echo ==================================================
echo OpenOCD 调试服务器已启动成功！
echo 现在可以用 Ozone / VSCode / ST-LINK 连接 localhost:3333
echo ==================================================
echo.
pause
exit /b 0